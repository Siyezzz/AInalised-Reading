...
export default function AdaptReader() {
  ...
  useEffect(() => {
    ...
    (async () => { try {
      ...
      if (!active) return;
      ticket.current = t;
      if (t.content) { setChapter(t.content); return; }
      const pending = localStorage.getItem(storageKey);
      if (pending) { setJobId(pending); setLabel('后台任务执行中，可以关闭页面稍后回来'); return; }
      const x = await call({ action: 'source', sourceText: t.sourceText });
      if (active) setPrepared(x.source);
    ...
    const poll = async () => {
      try {
        ...
        if (x.job?.status === 'complete' && x.job.output) {
          ...
          setChapter(x.job.output); await cache(x.job.output); localStorage.removeItem(storageKey); setJobId(null); setBusy(false); return;
        }
  ...
  return <article className="chapter-shell"><header className="chapter-intro"><a href="/shelf">返回书架</a><span>《{title}》第一章</span><h1>{chapter?.chapterTitle || prepared?.title || '准备你的阅读版本'}</h1></header>
    {busy && <p role="status">{label}，请保持页面打开。</p>}
    ...
    {prepared && !chapter && !busy && <section className="chapter-quiz"><h2>已取得原文 · {prepared.text.length.toLocaleString()} 字符</h2><details><summary>查看提取的第一章</summary><pre style={{ whiteSpace: 'pre-wrap', maxHeight: 320, overflow: 'auto' }}>{prepared.text}</pre></details><div><button onClick={generate}>开始改写并配图</button></div><p>后台会继续完成改写和配图，你可以关闭页面后再回来。</p></section>}
    {chapter && <><section className="chapter-body">{chapter.image && <img src={chapter.image} alt={chapter.imageCue?.prompt || '本章场景插图'} style={{ width: '100%', height: 'auto', borderRadius: 16 }} />}{chapter.chapter.map((p,i) => <p key={i}>{p}</p>)}</section>{chapter.originalEvidence?.map((x,i) => <details className="original-source" key={i}><summary><BookOpen size={18} />原文证据</summary><p>{x.adapted}</p><blockquote>{x.original}</blockquote><small>{x.note}</small></details>)}<section className="chapter-quiz"><h2>{chapter.quiz.question}</h2><div>{chapter.quiz.options.map((o,i) => <button key={i} onClick={() => setAnswer(i)}>{String.fromCharCode(65+i)}. {o}</button>)}</div>{answer !== null && <p>{answer === chapter.quiz.correctIndex ? chapter.quiz.rightFeedback : chapter.quiz.wrongFeedback[answer]}</p>}</section><button onClick={() => download(JSON.stringify(chapter, null, 2), `${title}-第一章.json`)}>下载本章</button>{!chapter.image && !busy && <button onClick={() => { setBusy(true); illustrate(chapter).catch(e => setError(String(e))).finally(() => setBusy(false)); }}>生成插图</button>}</>}
