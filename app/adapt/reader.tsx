...
export default function AdaptReader() {
  ...
  const [needExtraction, setNeedExtraction] = useState(false);
  ...
  if (error) {
    const uploadId = source.startsWith('upload:') ? source.slice('upload:'.length) : '';
    return <section className="adapt-state"><span>这次没有准备好</span><h1>{title}</h1><p>{error}</p><div>{uploadId && <a href={`/pdf?id=${encodeURIComponent(uploadId)}&title=${encodeURIComponent(title)}`}>查看原文 PDF</a>}<a href="/shelf">返回书架</a><a className="secondary" href="/discover">换一个版本</a>{source && !source.startsWith('upload:') && <a className="secondary" href={source} target="_blank" rel="noreferrer">查看来源记录 <ExternalLink size={14} /></a>}</div></section>;
  }
