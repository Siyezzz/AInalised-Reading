...
import { ADAPTATION_SKILL, AGNES_IMAGE_MODEL, AGNES_TEXT_MODEL, GROQ_TEXT_MODEL, IMAGE_MODEL, IMAGE_STYLE_PROMPT, TEXT_MODEL, fallbackSvg, generateAdaptedChapter, normalizeUserAiConfig } from './reading-workflow';
    ...
    try {
      ...
      if (body.action === 'image') {
        ...
        async function callAgnesImage() {
          ...
          const r = await fetch('https://apihub.agnes-ai.com/v1/images/generations', { method: 'POST', headers: { authorization: `Bearer ${env.AGNES_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: AGNES_IMAGE_MODEL, prompt: imagePrompt, n: 1, size: '1024x768' }), signal: AbortSignal.timeout(75_000) });
